...
Kpoints         =  50    # number of k-points along x and y in the 1st quadrant of the BZ

Ksym            =  1     # 1: only first quadrant of BZ
                         # 2: only upper half of BZ
                         # 3: only right half of BZ
                         # 4: full BZ

Gnum            =  2     # G-vectors along x and y: -Gnum, -Gnum+1, ..., Gnum-1, Gnum
...
Nbands          =  5     # If GSout = 3, plot this many bands
#-----------------------------------------------------------------------------#
#-----------------PARAMETERS FOR CALCULATING EPSILON(OMEGA)-------------------#
#-----------------------------------------------------------------------------#
eps_omega_calc  =  0     # calculate epsilon(omega)? (0=no, 1=yes)
...
Perturbation    =  0     # type of external perturbation:
                         # 0 = no perturbation
...
alpha_xc        =  0.0   # LRC kernel parameter (should be <= 0)
...
Dipole          = 0     # calculate the time-dependent macroscopic dipole moment

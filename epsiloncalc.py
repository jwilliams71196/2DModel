...
#-----------------------------------------------------------------------------#
def epsilon_t(Ksym,OccNum,Ntot,Kp_x,Kp_y,Mdim,dk,kval,Ct,C,ME,E,c,quasi2D):
                ...
                if OccNum[j1,i,j]==1:
                    for k in range(Ntot):
                        for l in range(Ntot):
                            ...
                            for m in range(Ntot):
                                if m != k:
                                    dum += ME[l,m,i,j]*ME[m,k,i,j]/(E[k,i,j] - E[m,i,j])
                            CC += dum*beta[j1,k,i,j]*np.conj(beta[j1,l,i,j])
            ck[i,j] = 2.*CC.real           
#
...
#
    ...
    CHI = -(2./c**2)*dum*dk**2/(2*np.pi)**2    # factor 2 because of spin  
    if Ksym==1:
        CHI = 4*CHI     
    if Ksym==2 or Ksym==3:
        CHI = 2*CHI
                ...
                if OccNum[i,k1,k2]==1:
                    for m in range(Ntot):
                        ...
                        for j in range (Mdim*Mdim):    
                            dum += Ct[i,k1,k2,j]*np.conj(C[m,k1,k2,j])
                        xi[i,m,k1,k2] = dum
            ...
            for i in range(Ntot):
                if OccNum[i,k1,k2]==1:
                    for n in range(Ntot):
                        for m in range(Ntot):
                            if m != n:
                               CC += np.conj(xi[i,n,k1,k2])*xi[i,m,k1,k2]\
                                *ME[n,m,k1,k2]
            ck[k1,k2] = CC.imag         
...
#   Now integrate over the Brillouin zone
    ...
    DIP = 2.*dum*dk**2/(2*np.pi)**2    # factor 2 because of spin  
    if Ksym==2 or Ksym==3:
        DIP = 2*DIP             
